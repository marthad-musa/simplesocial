  ...
  <div class="container my-4">
    <main class="form-signin w-100 m-auto text-center">
      <form method="POST" enctype="multipart/form-data">
        ...
        <!-- ---- IMAGE ---- -->
        <div class="form-floating fontCasper">
          <input type="email" name="email" value="<?=old_value('email')?>" class="form-control fontAlido" id="floatingInput" placeholder="name@example.com" disabled>
          <label class="fontDigital" for="floatingInput">Email address</label>
          <!-- ---- Errors ---- -->
          <?php if(!empty($data['user']->errors['image'])):?>
            <div class="text-danger text-center my-1 fontJordan">
              <?=$data['user']->errors['image']?>
            </div>
          <?php endif?>
          <!-- -| ./Errors\. |- -->
        </div>
